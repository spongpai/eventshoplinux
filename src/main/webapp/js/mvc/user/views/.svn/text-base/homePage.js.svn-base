...
var mask = $('#maskMethodVal option:selected').val();
            ...
            for (var p=0;p<output.length;p++) {
          		var newDiv = "";
          		 if(m == 0){
          	      ...
          	      m++;          	   
          	      divName = "usCon"+ output[p].queryId;
          
          	      newDiv += '<div class="item '+active+'" id='+divName+' style="width: 960px;"></div>'; 
          	      divName = "#" +divName;
   ...
   /*   $.ajax({
        ...
        success : function(output){
          ...
          $("#mapstatus").hide();
            ...
            $(".carousel-control").removeAttr("style");
            $('#map-inner-block').append(newDiv);
            mapDataLoader(output,divName);
        	
        	//console.log("done with maploader "+m);
         ...
         error : function(data){
        	 //console.log("we are in error"+data);
        	 console.log("error00"+JSON.stringify(data));
        	 for(var i=0;i<data.length;i++){
        		 console.log("data array is as follows "+data[i]);
        	 }
 ...
 if($('#dsTheme').val()=="")
      ...
      else if(!(/^(http|ftp|https):\/\/[A-Za-z0-9_-]+\.+[A-Za-z0-9.\/%&=\?_:;-]+$/.test($('#dsPath').val()))){
      ...
      else if(!(/^[-+]?[0-9]+\.[0-9]+$/.test($('#dsUnitLat').val()))  && ($('#dsUnitLat').val()!="")) // can be -ve and decimal --sanjukta
      {
      ...
      else if(dataObj.srcFormat=="csv")
      {
        ...
        dataObj.supportedwrapper="CSV";
        dataObj.csvkeyvalue=$('#key-value_pair').val();
      ...
      console.log("wrapper: "+JSON.stringify(dataObj.wrapper));
      ...
      this.datasourceModel.save(null,
                {
                  ...
                  success: function (datasourceModel,responseText) {
                    ...
                    error: function(datasourceModel,responseText) {
                      alert("Data saved successfully error"); // need to pass json or accept string otherwise goes to error -- sanjukta
                      $("#toPopup").fadeOut("normal");  
		              $("#backgroundPopup").fadeOut("normal");  
