...
public class DataSourceService {
	...
	@Path("/datasources")
	public String saveDataSource(DataSource source){
        DataSourceManagementDAO dao=new DataSourceManagementDAO();     
        String datasourceSrcId=dao.saveDataSource(source);
	...
	public DataSource getDataSource(@PathParam(value = "id") final String srcId){
		DataSourceManagementDAO dao=new DataSourceManagementDAO();
		System.out.println("WE are in fetchdatasource : ---"+srcId);
		int srcIdInt=new Integer(srcId).intValue();
		DataSource source=dao.getDataSource(srcIdInt);
		System.out.println("datasource name is here "+ source.getSrcName()+" THE DATA");
		return source;
	} 
