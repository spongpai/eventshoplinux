...
public class AlertProcessor 
{
			...
			double maxValSol2, String msg, RegisterServlet registerServlet)
	{
		...
		if(srcQuery.contains("q"))
		{
			...
			for(int i = 0; i < registerServlet.queries.size(); ++i)
			{
				if(((Query)registerServlet.queries.get(i)).queryID.equals(srcQuery))
				{
					params = ((Query)registerServlet.queries.get(i)).finalResolution;	
					this.probEmagePath =  registerServlet.context+"results//"+((Query)registerServlet.queries.get(i)).outputName+"_before.png";
				}
			}
		}
		...
		if(nearestDS.contains("q"))
		{
			...
			for(int i = 0; i < registerServlet.queries.size(); ++i)
			{
				if(((Query)registerServlet.queries.get(i)).queryID.equals(nearestDS))
				{
					params = ((Query)registerServlet.queries.get(i)).finalResolution;	
					this.solEmagePath =  registerServlet.context+"results/"+((Query)registerServlet.queries.get(i)).outputName+"_before.png";
				}
			}
		}
