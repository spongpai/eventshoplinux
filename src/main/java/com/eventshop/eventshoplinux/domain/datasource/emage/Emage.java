...
//		            {"geometry":{
//		            	"type":"Polygon",
//		            	"coordinates":[[[-121.6,40],[-121.6,40.2],[-121.4,40.2],[-121.4,40]]]
//		            			},
//		             "properties":{
//		            	"theme":"PM2_5_daily",
//		            	"value":9.6},
...
//		           	 {"geometry":{
//		           		 "type":"Polygon",
//		           		 "coordinates":[[[-122,39.8],[-122,40],[-121.8,40],[-121.8,39.8]]]
//		           				 },
//		           	  "properties":{
//		           		 "theme":"PM2_5_daily",
//		           		 "value":66.5},
		...
		for(int i = 0; i < image.length; i++){
			for(int j = 0; j < image[0].length; j++){
				...
				if(!Double.isNaN(image[i][j]) && image[i][j] > 0.0) {
					...
					geoMetries += "{\"geometry\":{\"type\":\"Polygon\", \"coordinates\":[[["+swLong+","+swLat+"],"
									+ "["+swLong+","+(swLat+latUnit)+"],"
									+ "["+(swLong+longUnit)+","+(swLat+latUnit)+"],"
									+ "["+(swLong+longUnit)+","+swLat+"]]]},"
									+ "\"properties\":{ \"theme\":\""+this.theme+"\",\"value\":"+image[i][j]+"},"
									+ "\"type\":\"Feature\"},";
				}
			}
		}
		geoMetries = geoMetries.substring(0, geoMetries.length() - 1);	// remove extra "," at the end of string
